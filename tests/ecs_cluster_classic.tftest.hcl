...
run "container_insights_test" {
  command = plan

  variables {
    # Define ECS cluster name with a unique suffix
    cluster_name            = "cltest-insights123"
    enable_cloudwatch_agent = true
    ...
    autoscaling_groups = [
      {
        ...
        user_data = <<-EOT
          #!/bin/bash
          echo ECS_CLUSTER=cltest-insights123 >> /etc/ecs/ecs.config
        EOT
      }
    ]
  ...
  assert {
    ...
    error_message = "Container Insights must be enabled for this test."
  }
